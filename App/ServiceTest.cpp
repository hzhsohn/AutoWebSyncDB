...
TCHAR szServiceName[] = _T("Hx-Kong AutoWebSyncDB Service");
BOOL bInstall;
...
void helpTip()
{
		...
		printf("本程序请使用如下命令行,服务启动后会检测AutoWebSyncDB.ini文件,\r\n");
		printf("文件不存在会自动生成,访问的网站和访问间隔请自行修改ini文件\r\n\r\n");

		printf("注:使用win7,win8以上系统时请确认CMD窗体是否为管理员权限运行,不然操作失败\r\n");
		printf("直接运行: -run\r\n");
		printf("安装服务: -install\r\n");
		printf("启动服务: -start\r\n");
		printf("停止服务: -stop\r\n");
		printf("卸载服务: -uninstall\r\n");
		printf("服务状态: -status\r\n");
		printf("帮助: -help\r\n");
		printf("------------------------------------------------------------------\r\n");
